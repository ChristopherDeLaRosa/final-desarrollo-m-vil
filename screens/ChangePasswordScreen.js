...
export default function ChangePasswordScreen({ navigation }) {
  ...
  const validateReset = () => {
    ...
    else if (form.newPassword !== form.confirmPassword) e.confirmPassword = 'No coinciden';
    setErrors(e);
    return Object.keys(e).length === 0;
  };

  const sendRecoverCode = async () => {
    if (!form.email) return setErrors(e => ({ ...e, email: 'El correo es requerido' }));
    if (!isEmail(form.email)) return setErrors(e => ({ ...e, email: 'Correo no válido' }));

    setSendingCode(true);
    try {
      await apiClient.post(API_ENDPOINTS.RECOVER, { correo: form.email });
      Alert.alert('Código enviado', `Revisa tu correo: ${form.email}`);
    } catch (e) {
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      Alert.alert('Contraseña actualizada', 'Tu contraseña ha sido restablecida.', [
        { text: 'OK', onPress: () => navigation.replace('Login') },
      ]);
    } catch (e) {
      Alert.alert('Error', e?.message || 'No fue posible restablecer la contraseña');
    } finally {
      setResetting(false);
    }
  };
    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.select({ ios: 'padding', android: undefined })}>
      <ScrollView style={styles.scrollView}>
        <View style={styles.header}>
          <Text style={styles.headerTitle}>Cambiar / Restablecer Contraseña</Text>
        </View>
          ...
          <Card style={styles.infoCard}>
            <Card.Content>
              ...
              <Text style={styles.infoText}>
                1) Ingresa tu correo y toca “Enviar código”.{'\n'}
                2) Revisa tu email, copia el código recibido.{'\n'}
                3) Escríbelo aquí, define tu nueva contraseña y confirma.
              </Text>
            </Card.Content>
          </Card>
