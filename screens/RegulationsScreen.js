...
import { Card, Title, Paragraph, ActivityIndicator, Chip, Button, TextInput } from 'react-native-paper';
...
export default function RegulationsScreen({ navigation }) {
  ...
  const fetchRegulations = async () => {
    try {
      ...
      if ((error.message || '').includes('401')) {
        ...
        Alert.alert('Error', error.message || 'No se pudieron cargar las normativas.');
      }
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleDownload = (url, title) => {
    if (!url) {
      Alert.alert('Sin documento', 'Esta normativa no tiene enlace de descarga.');
      return;
    }
    Alert.alert(
      'Descargar Documento',
      `¿Deseas descargar "${title}"?`,
      [
    ...
    >
      ...
      <View style={styles.content}>
        <Text style={styles.subtitle}>
          Marco legal y regulatorio para la protección del medio ambiente en República Dominicana
        </Text>
        ...
        <View style={styles.filters}>
          <TextInput
            ...
            right={<TextInput.Icon icon="magnify" onPress={fetchRegulations} />}
          ...
          <View style={styles.tipoChips}>
            {['', 'Ley', 'Decreto', 'Resolución', 'Norma'].map(t => (
              <Chip
                ...
                style={[
                  styles.tipoChip,
                  t ? { backgroundColor: tipo === t ? getTypeColor(t) : '#eee' } : {},
                ]}
                textStyle={{ color: t && tipo === t ? '#fff' : '#333', fontWeight: 'bold' }}
              >
                {t || 'Todos'}
              </Chip>
            ))}
        ...
        {regulations.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Button mode="outlined" onPress={() => { setSearch(''); setTipo(''); fetchRegulations(); }}>
              Limpiar filtros
            </Button>
          </View>
        ) : (
          regulations.map((reg) => (
            <Card key={reg.id} style={styles.regulationCard}>
              <Card.Content style={styles.regulationContent}>
                <View style={styles.regulationHeader}>
                  <View style={styles.chipsContainer}>
                    <Chip
                      style={[styles.typeChip, { backgroundColor: getTypeColor(reg.type) }]}
                      textStyle={styles.chipText}
                    >
                      {reg.type || '—'}
                    </Chip>
                    {reg.number ? (
                      <Chip style={styles.numberChip} textStyle={styles.numberChipText}>
                        N.º {reg.number}
                      </Chip>
                    ) : null}
                  </View>
                ...
                {reg.description ? (
                  <Paragraph style={styles.regulationDescription}>{reg.description}</Paragraph>
                ) : null}
                ...
                <View style={styles.regulationDetails}>
                  ...
                  <View style={styles.detailRow}>
                    ...
                    <Text style={styles.detailValue}>{formatDate(reg.createdAt)}</Text>
                  </View>
                </View>
...
const styles = StyleSheet.create({
  ...
  content: { padding: 15 },
  subtitle: { fontSize: 16, textAlign: 'center', marginBottom: 16, color: '#666', lineHeight: 22 },

  // filtros
  filters: { backgroundColor: 'white', borderRadius: 12, padding: 12, marginBottom: 16, elevation: 2 },
  ...
  regulationHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 },
  ...
  numberChip: { height: 28, backgroundColor: '#e0e0e0', alignSelf: 'flex-start' },
  numberChipText: { color: '#333', fontSize: 11, fontWeight: 'bold' },

  regulationTitle: { fontSize: 16, fontWeight: 'bold', color: '#333', marginBottom: 6, lineHeight: 22 },
  regulationDescription: { fontSize: 14, color: '#666', lineHeight: 20, marginBottom: 12 },

  regulationDetails: { backgroundColor: '#f9f9f9', padding: 12, borderRadius: 8, marginBottom: 12 },
