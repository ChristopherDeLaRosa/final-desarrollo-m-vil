...
export default function VideosScreen() {
  ...
  const fetchVideos = async () => {
    try {
      ...
      const cats = Array.from(new Set(list.map(v => v.category).filter(Boolean)));
  ...
  return (
    ...
    >
      ...
      <View style={styles.content}>
        <Text style={styles.subtitle}>Aprende sobre medio ambiente con nuestros videos educativos</Text>
        ...
        <View style={styles.filters}>
          <View style={styles.catsRow}>
            <Chip
              ...
              style={[styles.catChip, !categoria ? { backgroundColor: '#2E7D32' } : {}]}
              textStyle={{ color: !categoria ? '#fff' : '#333', fontWeight: 'bold' }}
            >
              Todas
            </Chip>
            {categoriaList.map((cat) => (
              <Chip
                ...
                style={[
                  styles.catChip,
                  categoria === cat ? { backgroundColor: getCategoryColor(cat) } : {},
                ]}
                textStyle={{ color: categoria === cat ? '#fff' : '#333', fontWeight: 'bold' }}
              >
                {cat}
              </Chip>
            ))}
          ...
          <Card key={video.id} style={styles.videoCard}>
            <View style={styles.videoThumbContainer}>
              {isStreamable(video.url) ? (
                ...
                <TouchableOpacity onPress={() => openVideoExternal(video.url, video.title)} activeOpacity={0.8}>
            ...
            <Card.Content style={styles.videoContent}>
              <View style={styles.videoHeader}>
                <Chip
                  style={[styles.categoryChip, { backgroundColor: getCategoryColor(video.category) }]}
              ...
              {!!video.description && <Paragraph style={styles.videoDescription}>{video.description}</Paragraph>}
...
const styles = StyleSheet.create({
  ...
  videoCard: { marginBottom: 20, elevation: 3, borderRadius: 12, overflow: 'hidden' },
  ...
  videoTitle: { fontSize: 18, fontWeight: 'bold', color: '#333', marginBottom: 8, lineHeight: 24 },
  videoDescription: { fontSize: 14, color: '#666', lineHeight: 20, marginBottom: 10 },
