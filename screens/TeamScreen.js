  ...
  const fetchTeam = async () => {
    try {
      ...
      const endpoint = departamento
        ? `${API_ENDPOINTS.EQUIPO}?departamento=${encodeURIComponent(
            departamento
          )}`
      ...
      <View style={styles.filters}>
        <View style={styles.deptRow}>
          <Chip
            ...
            style={[
              styles.deptChip,
              !departamento ? { backgroundColor: '#2E7D32' } : {}
            ]}
            textStyle={{
              color: !departamento ? '#fff' : '#333',
              fontWeight: 'bold'
            }}
          >
            Todos
          ...
          {departamentos.map((d) => (
            <Chip
              ...
              style={[
                styles.deptChip,
                departamento === d ? { backgroundColor: '#4CAF50' } : {}
              ]}
              textStyle={{
                color: departamento === d ? '#fff' : '#333',
                fontWeight: 'bold'
              }}
            >
              {d}
            ...
            <Card.Content>
              ...
              {expandedId === member.id && (
                <View style={styles.details}>
                  {!!member.bio && (
                    <Paragraph style={styles.bio}>{member.bio}</Paragraph>
                  )}
                </View>
              )}
