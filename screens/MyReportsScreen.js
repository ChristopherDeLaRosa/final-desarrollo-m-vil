...
export default function MyReportsScreen({ navigation }) {
  ...
  const enProcesoCount = reports.filter(r => r.status === 'En Proceso' || r.status === 'En Revisión').length;
        ...
        {reports.length === 0 ? (
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>
              ...
              ¡Ayúdanos a proteger el medio ambiente reportando daños ambientales!
            </Text>
            <Button
              mode="contained"
          ...
          <>
            <View style={styles.statsContainer}>
              ...
              <View style={styles.statItem}>
                <Text style={[styles.statNumber, { color: '#FF9800' }]}>{enProcesoCount}</Text>
                <Text style={styles.statLabel}>En Proceso</Text>
              </View>
              <View style={styles.statItem}>
                <Text style={[styles.statNumber, { color: '#4CAF50' }]}>{resueltosCount}</Text>
              ...
              >
                <Card style={styles.reportCard}>
                  <Card.Content style={styles.reportContent}>
                    ...
                    <Paragraph style={styles.reportDescription} numberOfLines={2}>
                      {report.description}
                    </Paragraph>
                    ...
                    {report.comment && (
                      <View style={styles.commentContainer}>
                        <Text style={styles.commentLabel}>Último comentario:</Text>
...
const styles = StyleSheet.create({
  ...
  emptyText: { fontSize: 16, color: '#666', textAlign: 'center', marginBottom: 20, lineHeight: 22 },
  newReportButton: { paddingVertical: 5 },
  statsContainer: {
    flexDirection: 'row', justifyContent: 'space-around', backgroundColor: 'white',
    padding: 20, borderRadius: 12, elevation: 3, marginBottom: 20,
  },
  ...
  reportTitle: { fontSize: 16, fontWeight: 'bold', color: '#333', marginBottom: 5, lineHeight: 20 },
  reportDescription: { fontSize: 14, color: '#666', lineHeight: 18, marginBottom: 10 },
  reportFooter: { borderTopWidth: 1, borderTopColor: '#eee', paddingTop: 10, marginBottom: 5 },
  reportDate: { fontSize: 12, color: '#888', marginBottom: 3 },
  commentContainer: { backgroundColor: '#f9f9f9', padding: 10, borderRadius: 6, marginTop: 5 },
  commentLabel: { fontSize: 12, color: '#666', fontWeight: 'bold', marginBottom: 3 },
